...
if __name__ == '__main__':
    ...
    print("working with pytorch ver: {}".format(torch.__version__))

    # Hyper Parameters
    parser = argparse.ArgumentParser()
    parser.add_argument('--batch_size', type=int, default=16, help='training batch size')
    parser.add_argument('--test_batch_size', type=int, default=16, help='training batch size')
    ...
    assert args.pre_train is not '', 'Must provide a path to a trained model'
    ...
    for i, data in enumerate(test_loader):
        ...
        if i % args.print_freq == 0:
            print('Test: [{0}/{1} , Acc. {2}]'.format(i, len(test_loader), test_acc.avg))
